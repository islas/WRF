...
def feature_restart_em_real( orch ):
  ...
  builds = [
            ( "build_make_em_real_gnu_debug_dm", "make" ),
            ( "build_cmake_em_real_gnu_debug_dm", "cmake" )
            ]
  for build, build_type in builds:
    for wrf_case in cases:
      init_wrf = run_wrf.InitWRF( f"restart_{wrf_case}_init_{build_type}" )
